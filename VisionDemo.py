...
img_list = ['img/test1.jpg','img/scape2.jpg','img/cal5.jpg','img/test4.jpg','img/scape1.jpg','img/cal2.jpeg','img/fall.jpg','img/test2.jpg', 'img/wedding.jpg']
img_num = 0
...
current_rotation, rot_inc = (0, 16)
current_rotation, rot_inc = (0, 1)
# VIEW TRACKER
vt = ViewTracker(img)
...
while running:
    ...
    if fourier_testing:
        ...
        if fourier_1d:
            ...
            # Get focus data
            f_data = f.get_data()
            orig_rgb = show_original_rgb(f_data)
            orig_rgb =  cv.resize(orig_rgb,(400,orig_rgb.shape[1]))
            cv.imshow("Original Channels", orig_rgb)

            # Get raw FFT data
            fft_data = fft_1d_rgb(f_data)

            # Get top N frequencies for each channel 
            rgb_recon = reconstruct_rgb(fft_data, N=reconstruct_n)
            merged_recon = rgb_recon[-1] # Save actual data before resizing
            # Show reconstructed channels
            rgb_recon =  cv.resize(rgb_recon,(400,rgb_recon.shape[1]))
            cv.imshow("RGB Reconstruction", rgb_recon)

            # Reconstruct image patch
            fourier_recon = f.reconstruct(custom_mem=merged_recon)
            fourier_recon = cv.resize(fourier_recon,(400,400))
            cv.imshow(f"Fourier Reconstruction", fourier_recon)
    ...
    if k == ord('i'):
        ...
        cv.imwrite(DIR+im1name,im1)
    # Show a full rotation of the current view
    # TODO: Save as video
    if k == ord('/'):
        rot_step_deg = 1
        rot_step_1 = int((rot_step_deg/360)*len(f.coords))
        print(f"step1: {rot_step_1}")
        if rot_step_1 % 4 != 0: rot_step_1 = rot_step_1 + (4 - (rot_step_1 % 4))
        print(f"step2: {rot_step_1}")
        rot_step = max(1,rot_step_1)
        rot_step = 16
        num_steps = len(f.coords) // rot_step
        data = f.get_data()
        data = np.squeeze(data)
        total = 0
        for s in range(num_steps):
            offset = s*rot_step
            print(f"[{s}/{num_steps}] Rotation: {offset}/{len(f.coords)}/{data.shape}")
            shifted = np.roll(data, offset,axis=0)
            shift_img = f.reconstruct(custom_mem=shifted)
            shift_img = cv.resize(shift_img,(400,400))
            cv.imshow("Rotation Loop",shift_img)
            cv.waitKey(5)
        if offset != len(f.coords):
            print(f"Making correction...")
            shift_img = f.reconstruct(custom_mem=data)
            shift_img = cv.resize(shift_img,(400,400))
            cv.imshow("Rotation Loop",shift_img)
            cv.waitKey(5)
        print(f"Rot step 1: {rot_step_1}, mod: {rot_step_1 % 4}, rotstep: {rot_step}")
        cv.waitKey(0)
        cv.destroyWindow("Rotation Loop")

    if k == ord(','):
        current_rotation -= rot_inc
        print(f"Rotating: {current_rotation}/{len(f.coords)}")
        data = f.get_data()
        data = np.squeeze(data)
        shifted = np.roll(data, current_rotation,axis=0)
        shift_img = f.reconstruct(custom_mem=shifted)
        shift_img = cv.resize(shift_img,(400,400))
        cv.imshow("Rotation Loop",shift_img)
    if k == ord('.'):
